...
const LIMITS = {
  ...
  /**
   * 세션(시드 발급) 한도. 이게 **게임을 시작하는 횟수**를 제한한다는 점을 명시한다.
   * 시드 발급은 공짜에 가깝고(CPU 없음) 부정행위도 막지 않는다 — 부정행위는 제출 한도와
   * 큐·월클럭이 막는다. 그래서 40회/시간(가족·CGNAT 이 같은 IP를 공유한다)은 너무 짧아서,
   * 넘긴 판은 시드를 못 받아 **그 판만 제출 불가**가 된다. 그런데도 네트워크는 정상이므로
   * 화면에는 초록불 + "오프라인" 이 동시에 떴다(사용자에게는 이해 못 할 모순으로 보였다).
   */
  tokenPerHour: 300,
  submitPerHour: 30,
  submitPerMin: 6,
