...
(function (root, factory) {
  ...
  const CATALOG = {
    ko: {
      ...
      'submit.reveal': '공유 링크에 내 이름을 표시', 'submit.go': '검증 요청', 'submit.offline': '오프라인 — 제출 불가',
      'submit.verifying': '서버가 리플레이를 다시 돌려보는 중…',
      'submit.verified': '✓ 검증 완료 — 서버 재시뮬레이션으로 점수가 그대로 재현되었습니다',
      'submit.flagged': '⚑ 검증 통과 · 상위권 검수 대상 (인간을 넘은 지표)',
      ...
      'mine.title': '내 기록 (이 기기)', 'mine.empty': '이 브라우저에 저장된 링크가 없습니다.',
      'mine.clear': '목록 지우기',
    ...
    en: {
      ...
      'submit.reveal': 'Show my name on the share link', 'submit.go': 'Verify & submit',
      ...
      'submit.flagged': '⚑ verified · queued for review (superhuman metrics)',
      ...
      'mine.title': 'My records (this device)', 'mine.empty': 'No saved links in this browser.',
    ...
    ja: {
      ...
      'board.anon': 'だれか', 'board.topAt': '提交当時 {rank}位', 'board.hold1': '1位維持 {time}',
      'submit.title': '記録提出', 'submit.name': '表示名（任意）', 'submit.namePh': '空欄なら自動コードネーム',
      'submit.reveal': '共有リンクに名前を表示', 'submit.go': '検証して提出', 'submit.offline': 'オフライン — 提出不可',
      'submit.verifying': 'サーバーがリプレイを再生成中…',
      'submit.verified': '✓ 検証完了 — スコアが完全に再現されました',
      'submit.soft': '観測指標のみ記録（レビュー対象外）',
      'submit.flagged': '⚑ 検証通過 · 上位は人手レビュー', 'submit.rejected': '✕ 却下',
      ...
      'mine.title': '自分の記録（この端末）', 'mine.empty': 'このブラウザに保存されたリンクはありません。',
      'mine.clear': '一覧を消す',
    ...
    zh: {
      ...
      'submit.reveal': '分享链接中显示我的名字', 'submit.go': '验证并提交', 'submit.offline': '离线 — 无法提交',
      'submit.verifying': '服务器正在重新模拟…',
      'submit.verified': '✓ 已验证 — 分数被完全复现',
      'submit.soft': '仅记录观察指标（不进入审查）',
      'submit.flagged': '⚑ 已验证 · 进入人工审查', 'submit.rejected': '✕ 被拒绝',
      ...
      'mine.title': '我的记录（本设备）', 'mine.empty': '此浏览器没有保存的链接。',
      'mine.clear': '清空列表',
