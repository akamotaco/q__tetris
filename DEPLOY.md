...
- **배포 후 클라이언트 반영**: `index.html` 은 `no-store`, JS/CSS 는 `max-age=3600` + `ETag` 재검증이다.
  그래서 새 버전 배포 후 휴대폰은 **새로고침 한 번**이면 새 코드로 뜬다(하드 리프레시 불필요).
  ETag 가 없던 시절에는 "새 HTML + 옛 client.js" 가 섞여서, 새 버튼은 화면에 뜨는데 클릭 코드가 없어
  **눌러도 아무 일이 일어나지 않는** 사고가 났다(모바일 `내 기록 ▸`). 회귀 고정: `verifytest` 「에셋 서빙」.
