...
(async function main() {
  ...
  const play = JSON.parse(await ev(`(async function(){
    ...
    /* 복사 버튼이 DOM 에 실재하는지 + 누르면 상태가 말하지 않는지. (한때 input 태그를 안 닫아서
       이 버튼이 DOM 에 아예 없었다 — 링크는 보이는데 복사할 수 없던 원인.) */
    const cb = document.querySelector('#subOut #copyBtn');
    let copyBtn = false, copyLabel = null;
    if (cb) { copyBtn = true; cb.click(); await new Promise(r => setTimeout(r, 120)); copyLabel = cb.textContent; }
    return JSON.stringify({
      nameWarn: nameWarn,
