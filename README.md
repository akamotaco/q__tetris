...
| `tools/e2e.js` | 실브라우저에서 플레이→녹화→**WebCrypto 서명→제출→서버 검증→명예의 전당 렌더→/r 링크→리플레이 재생→도전 레이스**, 새로고침 후 지문 유지, 내부 파일 비공개, **이름 노출 경고가 링크 생성 전에 보이는지, **재현/클라이언트 시간 대비가 리플레이에 보이는지**, 그리고 **보안 맥락 강하** — 같은 서버를 LAN IP 로 `http` 접속해(=폰에서 여는 그 경로) `crypto.subtle` 이 없어 제출·공유만 꺼지고 게임·보드 열람·시드 발급은 온전한지, 그 상태에서 제출 버튼이 사라지고 이유가 보이는지까지 실브라우저로 본다(localhost 는 예외로 보안 맥락이라 이 경로가 기존 테스트로 전혀 덮이지 않았다) 57 |
...
식별 방법: 코드네임이 떠야 할 자리에 **`서명 불가`** 라고 적힌다(길게 누르면 원인 설명). 예전에는 여기
"오프라인 — 제출 불가" 라고 떠서 **초록불과 모순되는 화면**을 만들었다 — 오프라인이 아니라 서명 불가다.
