...
> **이 저장소는 AI 코딩 에이전트와 세션으로 만들어졌다.**
> 기획·주문·검수는 사람이 하고, 코드·문서·테스트의 초안은 AI(pi 코딩 에이전트 + Qwen) 가 작성했다.
> 모든 결정과 수치에는 **근거가 커밋 메시지에 남고, 재현 가능한 테스트로 고정**되어 있다 —
> "AI 가 그럴듯하게 썼다"는 근거로 쓰이지 않도록 일부러 그렇게 했다.
> 미검증 항목은 별도 목록([UNVERIFIED.md](UNVERIFIED.md))으로 분리해 두었다.
...
형태 검사 → 내용 digest 중복 → 1회용 시드 확인 → 소유 서명 확인 → 시드 소진
   ...
   → 월클럭(게임 시간 ≤ 실제 경과) → verified → 이후 이상하면 사람이 검수 표시(⚑)
```

**왜 속도로 판정하지 않는가**: PPS·동작간격·반응시간은 계산해서 기록에 남기지만, 그것으로 사람과 도구를 나누지 않는다. 세계 상위권은 CTWL·ASD·롤킹 같은 손기술과 전용 자세로 "인간 한계" 문턱을 그냥 넘기고, 반대로 느리게 도는 도구는 그 문턱들을 아무 탈 없이 피한다 — 그 지표는 **사람을 벌하고 도구를 놓친다**. 자동 판정은 재시뮬 일치 하나만 쓰고, ⚑ 는 사람이 리플레이를 보고 붙인다(`tools/review.js --flag`).

**시간은 두 개 남긴다**: 순위가 매겨지는 **재현 시간**(서버가 같은 입력으로 다시 돌려본 값)과, 서버가 혼자 잰 **클라이언트 경과**(시드 발급→제출). 검증된 기록에서는 두 값을 재생 화면에 나란히 보여준다(순위는 계속 재현 시간 기준). 둘의 차이라야만 알 수 있는 것과 그래도 알 수 없는 것은 [SECURITY.md](SECURITY.md) 「시간 기반 어뷰지」.
...
| 200건 폭주 접수 | **0.7–1.9초** 안에 전원 "검증 중" 응답 (≈107–162건/초) |
| 200건 발행 완료 | **1.5–3.5초** (≈57–75건/초) |
| 그 사이 서버 이벤트 루프 지연 | **p95 12–14ms** — 검증이 정적 파일/보드 API 를 막지 않는다 |
| 큐가 밀린 정도 | 최대 깊이 192, 평균 대기 **0.9–1.5초** |

※ 같은 기계·같은 코드로 두 번 재면 접수 처리량이 162→107건/초 로 **2배 가까이 흔들렸다**(백그라운드·Defender·전원 계획 추정). 그래서 이 표는 범위로 적는다. 중요한 건 수치가 아니라 **"밀려도 사이트는 서고, 발행만 늦어진다"** 는 성질이고 그건 두 실측에서 일치했다.
...
### 서버가 없을 때 (`index.html` 더블클릭)

`file://` 는 보안 컨텍스트라서 WebCrypto·IndexedDB·localStorage 가 전부 살아있고 게임은 서버 없이도 온전히 돈다
(실측: `probe.js [8]`, 런타임/콘솔 에러 0). 서버가 필요한 쪽은 **조용히 꺼진다:**

| 기능 | 오프라인에서 |
| --- | --- |
| 플레이·점수·최고점·언어설정·효과음·숫자 폰트 | 그대로 동작 (최고점 등은 `localStorage`) |
| 기기 정체성(서명 키쌍) | 만들어지기는 한다. 다만 제출할 대상이 없어 할 일이 없고, origin 이 다르면(`file://` ↔ https) 애초에 다른 키다 |
| 세션·제출·월드 보드·명예의 전당·큐 | 요청을 보내지 않는다. `session()` 은 예외 대신 `null` 을 주고, 보드 쪽은 상태 문구만 보인다 |
| 판 종료 후 **공유 단계** | **건너뛴다.** 제출 버튼도 없는 상자에 이름칸만 띄워 "써놨는데 왜 안 되지" 를 만들지 않는다 |
| 그 판의 기록 | 이 기기에만 남고 `이 기기 기록 · 서버 미제출` 로 표시된다. 오프라인 판은 서버 시드로 태어나지 않아 나중에 제출도 불가 — 그래서 "나중에" 가 아니라 "미제출" 이 정확하다 |

### 글꼴 — 숫자만 별도 서브셋

점수·타이머·유지시간 같은 **숫자만** 위해 `Share Tech Mono` 를 숫자 글리프별로 잘라(3.4KB) `ui.css` 에 base64 로 인라인했다.

| 왜 이렇게 했나 |
| --- |
| 시스템 모노는 Windows / macOS / Android / iOS 가 전부 다르다. 같은 1,234점이 기종마다 생김과 **자폭**이 달라 보이니 HUD 를 신뢰하기 어렵다 |
| 인라인(데이터 URI)인 이유: `index.html` 을 더블클릭한 **오프라인 모드**에서도 같은 얼굴이어야 한다. 파일로 두면 그 모드에서 안 뜬다. 요청 수도 0 |
| `unicode-range` 로 범위을 제한했기 때문에 이 폰트에 없는 문자는 자동으로 `--mono` 로 떨어진다. 즉 **실패 모드가 없다** — 폰트가 못 뜨면 시스템 숫자가 보일 뿐이고 레이아웃은 그대로다 |
| 서브셋 만드는 법은 `ui.css` 주석에 있다(Google Fonts 의 `text=` 엔드포인트가 곧 서브셋 서비스라 도구가 필요 없다). 이 프로젝트의 "빌드 없음" 을 깨지 않는다 |

라이선스: SIL Open Font License 1.1 (Ralph du Carrois / Carrois Type Design). 전문은 `fonts/OFL-ShareTechMono.txt` 에 동봉했다(OFL 은 재배포 시 요구한다). 정적 파일 허용 목록에 없는 디렉터리라 서빙되지는 않는다.
`probe.js [8]` 이 로드·적용·"서브셋이 실제로 그려지는가"·그리고 글꼴 교체로 숫자 요소가 잘리지 않는지까지 검사한다.

## 테스트

```bash
npm test             # 코어 99 + 엔진/리플레이 99 + 서버 131 + 확장표 대조  (의존성·브라우저 불필요)
...
| `tools/selftest.js` | 결정론, 리플레이 왕복, **재시뮬 재현**, 점수·해시·틱 조작 탐지, T-스핀·lock out·O 회전·스폰 행, 규칙 버전, 모드/AI 기준선, **i18n 4개 언어 대칭**(키 누락·추가·{자리표시자}) 99 |
| `tools/verifytest.js` | HTTP 끝-끝-끝: 발행·기각·409 도용·시드 재사용·서명(DER/raw)·레이트리밋/IP 차단·이름 비노출(보드·**명예의 전당** 포함)·숨김 후에도 데이터 존재·순위/유지/기간 뷰·**워커 큐(티어/기아방지/복구/동시 대기 제한)**·**속도로 판정하지 않음(⚑ 는 사람만)**·**마이그레이션 업그레이드 경로**·**재현/클라이언트 시간 대비**·**에셋 서빙 200 보장**·**세션(시드) 한도에 걸리면 429 → 그 판은 제출 불가가 되는 경로** 136 |
| `probe.js` | 실브라우저에서 **`file://`** 로 돈다(오프라인 강하가 그 자체로 검증 대상이라). 키 입력→점수/줄/레벨/T-스핀/홀드/DAS, 캔버스 픽셀로 고스트·프리뷰 렌더 확인, 모바일 폭, 예외/콘솔 에러 0, **크로스 런타임 결정론**(같은 입력 스크립트를 브라우저 엔진과 노드 엔진이 각각 재시뮬 → 점수·틱·보드해시 완전 일치) |
| ⚠ 하네스 사이 틈 | probe 는 `file://`, e2e 는 `http` 라서 **"서버가 어떤 파일을 404 로 돌려준다"** 는 고장을 둘 다 못 본다. `ui.css` 가 그렇게 생긴 날부터 한 번도 서빙된 적이 없었다(페이지는 게임만 되는 껍데기로 렌더). 그래서 `verifytest` 에 **에셋 서빙 검정**(index.html 의 href/src 전부 200 + 올바른 content-type)을 넣었다 |
| `tools/e2e.js` | 실브라우저에서 플레이→녹화→**WebCrypto 서명→제출→서버 검증→명예의 전당 렌더→/r 링크→리플레이 재생→도전 레이스**, 새로고침 후 지문 유지, 내부 파일 비공개, **이름 노출 경고가 링크 생성 전에 보이는지, **재현/클라이언트 시간 대비가 리플레이에 보이는지**, 그리고 **보안 맥락 강하** — 같은 서버를 LAN IP 로 `http` 접속해(=폰에서 여는 그 경로) `crypto.subtle` 이 없어 제출·공유만 꺼지고 게임·보드 열람·시드 발급은 온전한지, 그 상태에서 제출 버튼이 사라지고 이유가 보이는지까지 실브라우저로 본다(localhost 는 예외로 보안 맥락이라 이 경로가 기존 테스트로 전혀 덮이지 않았다) 61 |
...
### 스마트폰에서 `http://192.168.x.x:8787` 로 열면 (제일 많이 빠지는 웅덩이)

브라우저는 **보안 맥락이 아닌 페이지에서 WebCrypto 를 잠근다.** 개인키 서명이 있어야 되는 기능은 전부
그 위에 서 있으므로, 서버가 코앞에서 잘 돌아가도(연결 표시는 초록불) 아래가 함께 꺼진다.

| 되는 것 | 안 되는 것 |
| --- | --- |
| 플레이 전체·소리· 터치 조작, 월드 보드·명예의 전당·기간 집계 열람, 공유 링크 **열기**/리플레이 재생 | 기록 **제출**, 공유 링크 **만들기**, 이 기기 기록 서버 동기화 (`/api/mine`) |

식별 방법: 연결 점이 **세 상태**입니다. 예전엔 두 상태(초록/회색)라 "서버는 살아있는데 서명만
안 되는" 상황을 표현하지 못했고, 그게 위 모순이 되었습니다. 설명은 점에 마우스를 올리거나 **누르면**
나옵니다(손가락에는 hover 가 없다). 코드네임 자리에 `서명 불가` 가 같이 뜨는 것이 노랑의 최유력 원인입니다.

| 점 | 뜻 | 무슨 일이 벌어지는가 |
| --- | --- | --- |
| 🟢 초록 | 서버 연결됨 · 제출 가능 | 전부 동작 |
| 🟡 노랑 | 서버는 닿지만 **이 브라우저/이 판은 제출 불가** | 플레이·보드 열람·링크 열기는 되고 제출·공유 만들기만 꺼진다. 이유는 점 설명에 적힌다 — 최유력 원인은 http 접속(서명 키 불가), 다음은 이 판이 시드를 못 받은 경우 |
| 🔴 빨강 | 서버에 **닿지 않음** | 게임은 계속 되고 판은 이 기기에만 남는다(`서버 미제출`) |

> 점 상태는 요청 성공에 파생된 기억만 믿지 않고 30초마다 `/api/health` 를 짚어 갱신합니다.

| 상황 | 처리 |
| --- | --- |
| 이 PC 에서만 | 설정 없음 — `http://localhost:8787` 은 예외로 보안 맥락 |
| 같은 네트워크 폰 | Caddy `tls internal` 로 https 한 포트 — [DEPLOY.md §4-B](DEPLOY.md) (폰에 인증서 경고 한 번) |
| 경고까지 없애기 | mkcert 사설 CA 를 폰에 설치 — [DEPLOY.md §4-C](DEPLOY.md) |
| 인터넷 공개 | 도메인 + Caddy 자동 인증서 — [DEPLOY.md §4-D](DEPLOY.md) |

> 이 동작은 추측이 아니라 검증 항목이다: e2e 가 LAN IP 로 http 접속을 열어 `crypto.subtle` 이 없고
> 제출만 꺼지고 게임은 그대로 되는지 확인한다. (Caddy·mkcert **설치 명령 자체**는 작성 환경에서
> 돌려보지 못해 UNVERIFIED 에 적어 뒷다.)
