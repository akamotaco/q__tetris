...
- **SRS + 월킥**(가이드라인 원문 테이블, y-up→화면 좌표 변환), 180° 킥(자체 확장), 7-bag, 홀드, 고스트, 락 딜레이 500ms(15회 리셋), 상단 버퍼 4행, 줄 삭제 연출
- **T-스핀 판정**(3-corner + 앞면 규칙 + 5번째 킥 만회, MINI 구분) · 가이드라인 스코어링 · B2B ×1.5 · 콤보 · 퍼펙트 클리어
- **규칙 버전(현재 `r3`)을 리플레이에 박아 둔다** — 규칙을 바꾸면 옛 리플레이가 검증 불가능해지므로, 바꿀 수 밖에 없는 상황에 대비한 최소한의 안전장치
...
### 규칙 버전 — 왜 리플레이에 `r3` 가 박혀 있나

엔진은 `RULES_ID`(현재 `r3`) 를 선언하고, 이것을 런 상태·`canonical()`·packed 리플레이·DB 컬럼에까지 찍는다.
...
- `NT1`(레거시) 은 `r0` 으로 읽히므로 포맷 자체가 깨지지는 않지만, 서버는 **현재 규칙(`r3`)만** 재현할 수 있어 기각된다.
...
아래 세 개는 버그가 아니라 **선택의 결과**다. 남들에게 보일 때 설명할 수 있게 미리 적어 둔다.

| # | 무엇 | 공식 서술 | 우리 | 실제로 갈리는 지점 |
| --- | --- | --- | --- | --- |
| 1 | **I 조각 킥 변형** | SRS 표준(=Tetris Worlds)과 **Arika(TGM3) 변형**이 따로 문서화된다 | 표준(Guideline) 쪽 | TGM3 기준으로 눈으로 비교할 때 I 의 4번째 시험이 다르게 작동한다. "SRS 표준"은 이쪽이 맞지만, 어떤 게임과 견주어 보느냐의 문제로는 남는다 |
| 2 | **필드 높이** | 놀이필드는 10×40 (보이는 20 + 숨은 20) | 10×24 (보이는 20 + 버퍼 4) | 보이는 판 **위 4행에 천장**이 있어서 그 위로는 킥이 거부된다. 열린 곳에서는 test#1 이 이겨 킥을 쓰지 않으니 실제 도달은 거의 불가능하지만, 40행을 문자 그대로 구현한 것과 갈리는 지점은 여기다. `core.js: BUFFER = 20` 으로 없앨 수 있다(렌더는 보이는 행만 그려 비용 0, 다만 보드 해시가 40행이 되므로 `RULES_ID` 를 올려야 한다) |
| 3 | **스폰 행** | Tetris Worlds 는 21–22행(은닉)에 스폰했고, 위키 스스로 "나중 게임은 1행 아래, 어떤 게임은 2행 아래" 라 적는다 | 조각의 윗칸이 **보이는 행 0** 에 걸리는 위치 | 가이드라인이 허용한 범위 안이지만 특정 게임과 1행 차이난다. 체감: 우리는 조각이 나온 순간 완전히 보이고, 저쪽은 윗칸이 잘려 나온다 |

한 줄로: **회전 알고리즘 자체는 표준에 맞고, 남은 차이는 필드 높이 하나(실질 영향 미미)와 테이블 변형·스폰 행 선택이다.**
...
```bash
npm test             # 코어 91 + 엔진 75 + 서버 94  (의존성·브라우저 불필요)
npm run test:browser # probe(게임/렌더링) + e2e(서버+브라우저 제출→검증→공유→재생) 32
...
| 테스트 | 본문을 무엇으로 증명하나 |
| --- | --- |
| `test.js` | 코어 규칙(회전/킥/판정/스코어링/버퍼·끝남 조건)과 **SRS 공식 테이블 호환**(원문 값 대조 + 좌표계 변환 + 참조 구현과 무작위 대조) 91 |
| `tools/selftest.js` | 결정론, 리플레이 왕복, **재시뮬 재현**, 점수·해시·틱 조작 탐지, T-스핀·lock out·O 회전, 규칙 버전, 모드/AI 기준선 75 |
| `tools/verifytest.js` | HTTP 끝-끝-끝: 발행·기각·409 도용·시드 재사용·서명(DER/raw)·레이트리밋/IP 차단·이름 비노출·숨김 후에도 데이터 존재·순위/유지/기간 뷰·**워커 큐(티어/기아방지/복구/동시 대기 제한)** 94 |
